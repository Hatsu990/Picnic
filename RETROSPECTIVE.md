...
## 2026-06-20 - 전체 사이트 디자인 1차 정리

- 사용자 의도: 결제/문자 발급은 나중으로 미루고, 소풍 사이트 전체 디자인을 먼저 컨설턴트 관점으로 정리하기를 원했다.
- 진행한 요청: 홈, 메뉴, 예약, 조회, 마이페이지, 관리자 화면에 공통으로 적용되는 디자인 기준을 세우고 `DESIGN.md`를 추가했다.
- 확인한 원인과 단서: 기존 UI는 기능은 갖췄지만 프로토타입 느낌이 강했고, 카페 예약 사이트로서 실제 메뉴 사진과 예약 CTA가 더 먼저 보여야 했다.
- 적용한 변경 사항: 홈 히어로를 실제 메뉴 이미지 배경과 예약 CTA 중심으로 재구성하고, 메뉴 카드/폼/버튼/상태 배지/관리자 화면의 색상과 간격을 통일했다.
- 검증 결과: `npm run typecheck`, `npm run build`가 통과했다. Chrome 캡처로 홈, 메뉴, 예약 화면의 데스크톱/모바일 레이아웃을 확인했고 큰 겹침이나 잘림은 없었다.
- 다음 주의사항: 관리자 메뉴 화면은 33개 메뉴가 세로 카드로 길게 이어져 운영성이 떨어질 수 있으므로 다음 단계에서 표형 편집 화면으로 압축하는 것이 좋다.

## 2026-06-23 - 카페 오더형 2차 디자인 방향 확정

- 사용자 의도: 1차 디자인 커밋은 시안으로 보존하고, gstack식 질문/비교 후 선택한 방향으로 2차 리디자인을 진행하기를 원했다.
- 진행한 요청: A안인 카페 오더형을 확정하고, 홈 히어로를 실제 포케 사진 중심으로 조정했다.
- 확인한 원인과 단서: 1차 시안의 추상적 히어로보다 실제 메뉴 사진이 소풍의 메뉴 예약 서비스 성격을 더 빠르게 전달했다.
- 적용한 변경 사항: 히어로 배경 이미지를 실제 연어 포케 사진으로 바꾸고, 메뉴 카드 hover와 가격 영역의 시각적 밀도를 보강했다. `DESIGN.md` 결정 로그에 A안 확정을 기록했다.
- 검증 결과: `npm run typecheck`, `npm run build`가 통과했다. Chrome 캡처로 홈 데스크톱/모바일을 확인했고 텍스트 가림이나 CTA 잘림은 없었다.
